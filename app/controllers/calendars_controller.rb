class CalendarsController < ApplicationController
  ...
  def create
    ...
    (date_from..date_to).each do |date|
      calendar = Calendar.where(motorbike_id: params[:motorbike_id], day: date)

      if calendar.present?
        calendar.update_all(price: calendar_params[:price], status: calendar_params[:status])
      else
        Calendar.create(
          motorbike_id: params[:motorbike_id],
          day: date,
  ...
  def host
    ...
    params[:motorbike_id] ||= @motorbikes[0] ? @motorbikes[0].id : nil

    if params[:q].present?
      params[:start_date] = params[:q][:start_date]
      params[:motorbike_id] = params[:q][:motorbike_id]
    end

    @search = Reservation.ransack(params[:q])

    if params[:motorbike_id]
      ...
      @events = @motorbike.reservations.joins(:user)
      ...
      @days = Calendar.where("motorbike_id = ? AND day BETWEEN ? AND ?", params[:motorbike_id], first_of_month, end_of_month)
    else
      @motorbike = nil
      @events = []
      @days = []
