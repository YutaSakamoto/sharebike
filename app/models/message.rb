...
<<<<<<< HEAD
        Notification.create(content: "#{sender.fullname}さんから新しいメッセージがありました", user_id: self.conversation.recipient_id)
      else
        sender = User.find(self.conversation.recipient_id)
        Notification.create(content: "#{sender.fullname}さんから新しいメッセージがありました", user_id: self.conversation.sender_id)
=======
